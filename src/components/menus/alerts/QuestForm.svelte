	...
	<section class="flex flex-col gap-3 rounded-md border bg-card p-4 shadow-sm">
		...
		{:else if isItem}
			<label class="flex flex-col gap-1 text-xs text-muted-foreground">
				...
				<Input
					...
					value={itemId}
					oninput={(e: Event) =>
						(itemId = clampInt(Number((e.target as HTMLInputElement).value), 0, 100000))}
					class="w-32"
				/>
			</label>
		{/if}
		...
		<label class="flex flex-col gap-1 text-xs text-muted-foreground">
			<span class="font-medium uppercase tracking-wide"
				>Minimum amount{rewardType === 3 ? " (stardust)" : ""}</span
			>
			<Input
				type="number"
				min="0"
				value={amount}
				oninput={(e: Event) =>
					(amount = clampInt(Number((e.target as HTMLInputElement).value), 0, 1000000))}
				class="w-32"
			/>
		</label>
	</section>

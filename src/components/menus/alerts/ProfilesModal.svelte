<script lang="ts">
	...
	// A bottom-sheet profile manager on the new $lib/drawer. Body is a uniform
	// accordion of CollapsibleSections (Profile / Location / Schedule) matching
	// the rest of the tooling. The Profile section switches in place (no reload)
	// and creates new profiles; Location editor is injected as a snippet so this
	// component stays decoupled from the location/area pickers.
	//
	// Fixed-height bottom sheet (h-[85dvh]) with an internal scroll — it does NOT
	// grow as accordion sections open; the body scrolls. On-screen-keyboard
	// handling now comes from Drawer.VirtualKeyboardProvider; the old
	// #pokedex-drawer-pin !important hack that fought vaul's keyboard handler is
	// gone (the new drawer never writes inline height/bottom).
	let {
		open = $bindable(false),
		profiles,
		currentProfileNo,
	...
	// Uniform accordion — every section starts collapsed.
	...
	// A profile with no areas and no point has nowhere to fire alerts.
	...
	<Drawer.Portal>
		...
		<Drawer.VirtualKeyboardProvider>
			...
			>
				...
				>
					<!-- Borderless rows à la the hoopa hotspot picker: no
						per-item borders, selection carried by a primary fill so
					...
					<div class="flex flex-col gap-1">
						...
						<!-- New-profile row — same chrome as the list; Plus icon
							hints "add" (mirrors hoopa's custom-location row). -->
						<button
							type="button"
				...
				<!-- Active profile settings — same dropdown chrome as above -->
				{#if activeProfile}
					<CollapsibleSection
						icon={MapPin}
						title="Location"
					...
					>
						{#key currentProfileNo}
							<ActiveHoursEditor profile={activeProfile} saving={savingHours} onSave={onSaveHours} />
						{/key}
					</CollapsibleSection>
				{/if}
			</Drawer.Content>

<script lang="ts">
	...
	function getPreviewImageUrl(url: string) {
		if (!url) return "";
		return url.startsWith("data:") ? url : resize(url, { width: 64 });
	}
	...
	let focusIconUrl = $derived(getPreviewImageUrl(iconUrl ?? ""));
	let companionIconUrls = $derived(
		companionPokemon.map((pokemon) =>
			getPreviewImageUrl(getIconPokemon({ pokemon_id: pokemon.pokemon_id, form: pokemon.form }))
		)
	);

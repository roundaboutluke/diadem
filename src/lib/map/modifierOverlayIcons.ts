...
export function getModifierOverlayImageOffset(
	baseImageOffset: number[] = [0, 0],
	baseImageSize: number,
	overlayImageSize: number
) {
	if (overlayImageSize === 0) return baseImageOffset;

	const offsetScale = baseImageSize / overlayImageSize;
	return [
		baseImageOffset[0] * offsetScale,
		baseImageOffset[1] * offsetScale
	];
}
